...
class BTreeV2:
    def __init__(self, t: int, max_elements: Optional[int] = None):  # NOVO: max_elements
    ...
    def set_search_step_limit(self, limit: Optional[int]):  # NOVO MÉTODO
        ...
        if limit is not None:
            print(f"INFO (BTreeV2): Limite de busca definido para {limit} passos.")
        else:
            print("INFO (BTreeV2): Limite de busca removido.")
    ...
    Optional[Moto]:
        ...
        passos_ref[0] += 1
        if self._search_step_limit is not None and passos_ref[0] > self._search_step_limit:  # NOVO: Checa limite
            return None
    ...
    def _split_child(self, parent_node: BTreeNodeV2, child_index: int):
        # ... (código de _split_child como na versão anterior de b_tree_v2.py) ...
        full_child_node = parent_node.children[child_index]
        if full_child_node is None or full_child_node.n != (2 * self.t - 1): return
    ...
    def _insert_non_full(self, node_to_insert_in: BTreeNodeV2, key_to_insert: Moto) -> bool:
        # ... (código de _insert_non_full como na versão anterior de b_tree_v2.py) ...
        ...
        if node_to_insert_in.leaf:
            while i >= 0 and node_to_insert_in.keys[i] is not None and key_to_insert < node_to_insert_in.keys[i]:
                ...
                i -= 1
            if (i + 1 < node_to_insert_in.n) and node_to_insert_in.keys[i + 1] is not None and node_to_insert_in.keys[
                i + 1] == key_to_insert: return False
            node_to_insert_in.keys[i + 1] = key_to_insert;
            node_to_insert_in.n += 1
            return True
        else:
            while i >= 0 and node_to_insert_in.keys[i] is not None and key_to_insert < node_to_insert_in.keys[i]: i -= 1
            i += 1
            child_to_descend = node_to_insert_in.children[i]
            if child_to_descend is None: return False
            if child_to_descend.n == (2 * self.t - 1):
                self._split_child(node_to_insert_in, i)
                if key_to_insert > node_to_insert_in.keys[i]: i += 1
            return self._insert_non_full(node_to_insert_in.children[i], key_to_insert)

    def inserir(self, key_to_insert: Moto) -> bool:  # Modificado para retornar bool
        if self.max_elements is not None and self._count >= self.max_elements:  # NOVO: Checa M1
            # print(f"AVISO (BTreeV2): Capacidade máxima de {self.max_elements} elementos atingida. Não inserindo {key_to_insert.nome}.")
    ...
    def remover(self, key_to_remove: Moto) -> bool:  # Mantido como placeholder
        # print(f"Aviso: Remoção na BTreeV2 é placeholder e não remove o item {key_to_remove.nome}.")
        # Para o benchmark, simula que não encontrou ou não removeu.
        # Não decrementa self._count aqui, a menos que a remoção real seja implementada.
        return False

    def _traverse_recursive(self, node: Optional[BTreeNodeV2], result_list: List[Moto], max_items: int):
        # ... (código de _traverse_recursive como na versão anterior de b_tree_v2.py) ...
    ...
    def exibir(self) -> None:
        # ... (código de exibir como na versão anterior de b_tree_v2.py) ...
